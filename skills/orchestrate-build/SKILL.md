...
export PINNED_BASE_SHA="${pinned_base_sha:-$(git rev-parse origin/HEAD 2>/dev/null || git rev-parse origin/main)}"
...
instructed to **invoke `implement-spec` (`skills/implement-spec/SKILL.md`) and follow it completely** (full
rigor — its ledger, self-review, gap analysis, and verification are the bar), with inputs:
...
- **headless** — hand off to `scripts/drive-build.sh` (or invoke `<agent_cmd> "<the invocation above>"` in the
  worktree yourself); the child is a fresh top-level session with its own full window. Parse the PR + evidence
  from its output and the ledger.
- **subagent** — spawn one subagent with that invocation as its prompt; it returns the compact evidence report
  (its own implementation noise stays out of your context). Nesting is deep enough that `implement-spec`'s own
  fresh-context review still runs inside it.
...
### 2.5 Continue
Re-read `pauseRequested` from the ledger (the human may have set it mid-run). If false and `autonomy` permits,
proceed to the next ticket **in a fresh context** — no human re-invocation needed. Emit a one-line progress
update (§4) at each boundary.
